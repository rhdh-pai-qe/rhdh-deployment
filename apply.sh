...
export LIGHTSPEED_FRONTEND_TAG="${LIGHTSPEED_FRONTEND_TAG:-ghcr.io/redhat-developer/rhdh-plugin-export-overlays/red-hat-developer-hub-backstage-plugin-lightspeed:bs_1.39.1__0.5.7}"
export LIGHTSPEED_BACKEND_TAG="${LIGHTSPEED_BACKEND_TAG:-ghcr.io/redhat-developer/rhdh-plugin-export-overlays/red-hat-developer-hub-backstage-plugin-lightspeed-backend:bs_1.39.1__0.5.7}"
...
$binary build $mode | envsubst '${RHDH_NAMESPACE},${RHDH_URL},${LIGHTSPEED_FRONTEND_TAG},${LIGHTSPEED_BACKEND_TAG},${INSIGHTS_IMAGE_TAG}' | sed "s/$GH_PLACEHOLDER/$GH_PRIVATE_KEY/" | oc apply -f -

oc rollout status deployment rhdh && echo "RHDH running at: ${RHDH_URL}"
